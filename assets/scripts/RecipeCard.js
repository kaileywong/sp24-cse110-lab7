...
class RecipeCard extends HTMLElement {
	...
	set data(data) {
		...
		articleImg.src = data.imgSrc;
		articleImg.alt = data.imgAlt;

		const title = document.createElement('p');
		title.className = 'title';
		const article = document.createElement('a');
		article.href = data.titleLnk;
		article.textContent = data.titleTxt;
		...
		const numStars = document.createElement('span');
		const ratingNum = data.rating;
		numStars.textContent = ratingNum;
		const ratingImg = document.createElement('img');
		ratingImg.src = `./assets/images/icons/${ratingNum}-star.svg`;
		ratingImg.alt = `${ratingNum} stars`;
		const numRatings = document.createElement('span');
		numRatings.textContent = `(${data.numRatings})`;
		ratingDiv.append(numStars, ratingImg, numRatings);
		...
		articleEl.append(articleImg, title, org, ratingDiv, time, ingredients);
